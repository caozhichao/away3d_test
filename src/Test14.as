package
{
	...
	public class Test14 extends Sprite
	{
...
//		private var lens:OrthographicLens;
		...
		private var pMapX:int = 400;
		private var pMaxpY:int = 300;
...
//			testMD2();
		...
		{
			//计算2d坐标
			var p2d:Point = p3dTo2d(evt.scenePosition);
			trace(p2d);
			var offX:int = p2d.x - pMapX;
			var offY:int = p2d.y - pMaxpY;
			pMapX = p2d.x;
			pMaxpY = p2d.y;
			trace(offX,offY);
			//计算滚屏
			var p:Point = windowScroll2.scroll(new Point(pMapX,pMaxpY));
			var cameraIsMove:Boolean = false;
			if(p.x != pMap.x)
			{
				cameraIsMove = true;
			}
			...
			view3d.camera.y = p.y;
			//移动人物
			plane.moveTo(plane.x + offX,plane.y - offY,plane.z);
		}
